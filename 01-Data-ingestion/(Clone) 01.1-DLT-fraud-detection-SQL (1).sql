...
-- MAGIC %md
-- MAGIC Your DLT Pipeline has been installed and started for you! Open the <a dbdemos-pipeline-id="dlt-fsi-fraud" href="#joblist/pipelines/9c690e9c-75bf-4653-a170-a21bfcfca6bb" target="_blank">Fraud detection Delta Live Table pipeline</a> to see it in action.<br/>
...
-- MAGIC %python
-- MAGIC display(spark.read.json(f'/Volumes/{catalog}/{schema}/fraud_raw_data/transactions'))
...
-- MAGIC %python
-- MAGIC display(spark.read.csv(f'/Volumes/{catalog}/{schema}/fraud_raw_data/customers', header=True, multiLine=True))

-- COMMAND ----------

-- DBTITLE 1,Raw incoming country
-- MAGIC %python
-- MAGIC display(spark.read.csv(f'/Volumes/{catalog}/{schema}/fraud_raw_data/country_code', header=True))
...
AS 
  SELECT * FROM cloud_files("/Volumes/${catalog}/${schema}/fraud_raw_data/transactions", "json", map("cloudFiles.maxFilesPerTrigger", "1", "cloudFiles.inferColumnTypes", "true")) 
...
AS 
  SELECT * FROM cloud_files("/Volumes/${catalog}/${schema}/fraud_raw_data/customers", "csv", map("cloudFiles.inferColumnTypes", "true", "multiLine", "true"))
...
AS 
  SELECT * FROM cloud_files("/Volumes/${catalog}/${schema}/fraud_raw_data/country_code", "csv")

-- COMMAND ----------

-- MAGIC %md
-- MAGIC # **_TASK 1 Start_**
-- MAGIC
-- MAGIC Ingest the csv files that is located at the volume path: /Volumes/"yourcatalog"/"yourschema"/fraud_raw_data/fraud_report

-- COMMAND ----------

-- DBTITLE 1,Fraud report (labels for ML training)
/* Solution below, delte before sharing with participants*/
CREATE STREAMING LIVE TABLE fraud_reports
AS 
  SELECT * FROM cloud_files("/Volumes/${catalog}/${schema}/fraud_raw_data/fraud_report", "csv")
...
-- MAGIC %md
-- MAGIC # **_# Task 2 Start_**
-- MAGIC
-- MAGIC
-- MAGIC You need to ensure good data quality. 
-- MAGIC Please implement in the following code snippe: 
-- MAGIC 1. correct_data should not be null **and** 
-- MAGIC 2. correct_customer_id should not be null
-- MAGIC
-- MAGIC Leverage the code snippet below (remove the comment tags /* */ and add the constraints
-- MAGIC )
-- MAGIC
-- MAGIC
-- MAGIC
...
CREATE STREAMING LIVE TABLE silver_transactions (
  /* DEFINE THE 2 REQUIRED CONSTRAINTS HERE*/
  CONSTRAINT 1... YOUR CODE
  CONSTRAINT 2... YOUR CODE
)
AS 
...
-- MAGIC
-- MAGIC Open the <a dbdemos-pipeline-id="dlt-fsi-fraud" href="#joblist/pipelines/9c690e9c-75bf-4653-a170-a21bfcfca6bb" target="_blank">Fraud detection Delta Live Table pipeline</a> and click on start to visualize your lineage and consume the new data incrementally!
